...
schedule(void)
{
	...
	unsigned int priority = 0xffffffff; //priority for lower number
	...
	//priority scheduling algorithm
		...
		for(i; i < NPROCS; i++)
		  {
		    if (proc_array[i].p_state == P_RUNNABLE)
		      {
			if(proc_array[i].p_prior < priority)
			  priority = proc_array[i].p_prior;
		      }
		  }
		pid = (pid + 1) % NPROCS; 
		if (proc_array[pid].p_state == P_RUNNABLE)
		  if(proc_array[pid].p_prior <= priority)    
		    run(&proc_array[pid]);
	      }
	  }
	else if(scheduling_algorithm == 3)
	  {
		 ...
		 if (proc_array[pid].p_state == P_RUNNABLE) {
		// check if its over the time limit
